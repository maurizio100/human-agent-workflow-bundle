#!/usr/bin/env python3
"""Work-tracking façade — work items & epics.
...
adapters/<active>/tracker.py. See SKILL.md for the operation vocabulary.
"""
...
def main():
    ...
    target = os.path.join(_HERE, "adapters", adapter, "tracker.py")
    if not os.path.exists(target):
        sys.stderr.write(
            f"error: work-tracking adapter '{adapter}' has no tracker.py at {target}\n"
        )
        sys.exit(1)
