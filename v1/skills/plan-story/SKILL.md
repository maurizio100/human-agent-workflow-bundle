...
1. Read `specs/STORIES.md` — the story index.
2. Filter to stories with status `draft`, `ready`, or `in-progress`.
...
## Story
STORY-NNN: <title>
Link: specs/stories/STORY-NNN-<slug>.feature
...
docs/arc42/08-crosscutting.md. Use the # Layer: header from the feature file to determine
...
1. Verify story status is `ready` or `draft`. If `in-progress`, stop — a branch already exists.
2. Flip `# Status: ready` → `# Status: in-progress` in the `.feature` file. Also run
   `cd "$(git rev-parse --show-toplevel)" && python3 .claude/skills/plan-story/mark-in-progress.py STORY-NNN`. Commit both changes
   directly to the default branch: `docs: mark STORY-NNN as in-progress`.
